...
from VAE.models import VAEGAN_IGLS
from VAE.dataloader import LongDataset, SubjectBatchSampler
from VAE.train import lvaegan_loss, d_loss
...
slope = True if "SLOPE" in params and params["SLOPE"].lower() == 'true' else False
min_subj_t = None if "MIN_DATA" not in params else int(params["MIN_DATA"])
include_a01 = True if "INLCUDE_A01" in params and params["INLCUDE_A01"].lower == 'true' else False
...
for epoch in range(pre_epochs, pre_epochs + epochs):
    ...
    if (epoch + 1) % save_epochs == 0:
        ...
        print(f'Saved {name}_{epoch + 1}.h5')
