...
nu = 1 if "NU" not in params else float(params["NU"])
lr = float(params["LR"]) if "LR" in params else 1e-4
d_lr = float(params["D_LR"])*lr if "D_LR" in params else lr
momentum = float(params["MOMENTUM"]) if "MOMENTUM " in params else 0.9
...
igls_iterations = int(params['IGLS_ITERATIONS']) if 'IGLS_ITERATIONS' in params else None
...
vae_parameters = list(model.encoder.parameters()) + \
                 ...
                 list(model.linear_var.parameters()) + \
